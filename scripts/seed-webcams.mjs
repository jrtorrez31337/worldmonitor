...
const PREFIX = '';  // no prefix for self-hosted
const WINDY_BASE = 'https://api.windy.com/webcams/api/v3/webcams';
const PAGE_LIMIT = 50;
const BATCH_SIZE = 500;
const GEO_TTL = 86400;  // 24 hours
...
async function fetchRegion(bounds, regionName) {
  ...
  if (offset >= 9950 && cameras.length >= 9950) {
...
async function main() {
  ...
  // Write full list as JSON for future feature use
  const fullKey = `${PREFIX}webcam:cameras:full:${version}`;
  console.log(`  Writing full list (${unique.length} cameras)...`);
  await pipelineRequest([['SET', fullKey, JSON.stringify(unique)]]);

  // Set TTL on data keys
  await pipelineRequest([
    ['EXPIRE', geoKey, String(GEO_TTL)],
    ['EXPIRE', metaKey, String(GEO_TTL)],
    ['EXPIRE', fullKey, String(GEO_TTL)],
  ]);
  ...
  if (prev && String(prev) !== String(version)) {
    await pipelineRequest([
      ...
      ['DEL', `${PREFIX}webcam:cameras:full:${prev}`],
    ]);
