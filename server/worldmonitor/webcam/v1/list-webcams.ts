...
const RESPONSE_CACHE_TTL = 86400; // 24 hours
...
// Quantize bbox to 1° grid to maximize cache hits across nearby viewports
function quantizeBbox(w: number, s: number, e: number, n: number): string {
...
export async function listWebcams(_ctx: ServerContext, req: ListWebcamsRequest): Promise<ListWebcamsResponse> {
  const { zoom = 3, boundW = -180, boundS = -90, boundE = 180, boundN = 90 } = req;
  ...
  const cacheKey = `webcam:resp:${version}:${zoom}:${quantizeBbox(boundW, boundS, boundE, boundN)}`;
  ...
  // Compute center and dimensions for GEOSEARCH
  const centerLat = (boundN + boundS) / 2;
  const heightKm = Math.abs(boundN - boundS) * 111.32;

  // Antimeridian: if W > E, split into two queries
  let ids: string[];
  if (boundW > boundE) {
    // antimeridian path — computes per-side widths independently
    const centerLon1 = (boundW + 180) / 2;
    const centerLon2 = (-180 + boundE) / 2;
    const width1 = (180 - boundW) * 111.32 * Math.cos(centerLat * Math.PI / 180);
    const width2 = (boundE + 180) * 111.32 * Math.cos(centerLat * Math.PI / 180);
    ...
    const centerLon = (boundW + boundE) / 2;
    const widthKm = equirectangularWidthKm(boundS, boundN, boundW, boundE);
  ...
  setCachedJson(cacheKey, result, RESPONSE_CACHE_TTL).catch(() => {});
