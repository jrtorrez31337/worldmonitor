...
export class MapComponent {
  ...
  private renderOverlays(projection: d3.GeoProjection): void {
    ...
    if (this.state.layers.webcams && this.webcamData.length > 0 && this.state.zoom >= 2) {
      ...
      this.webcamData.forEach((cam: any) => {
        const lon = cam.lng ?? cam.lon ?? 0;
        const lat = cam.lat;
        const pos = projection([lon, lat]);
        if (!pos) return;
        const isCluster = 'count' in cam;
        const radius = isCluster ? Math.min(4 + Math.sqrt(cam.count), 12) : 3;
        const size = radius * 2;
        const color = CATEGORY_COLORS[cam.category] ?? '#888888';
        const dot = document.createElement('div');
        ...
        dot.title = isCluster ? `${cam.count} webcams` : (cam.title ?? cam.category ?? 'Webcam');
        dot.addEventListener('click', (e) => {
          e.stopPropagation();
          if (isCluster) {
            this.showWebcamClusterPopup(cam, e.clientX, e.clientY);
          } else {
            this.showWebcamTooltip(cam, e.clientX, e.clientY);
          }
        });
        this.overlays.appendChild(dot);
      });
  ...
  private showWebcamTooltip(cam: any, clientX: number, clientY: number): void {
  ...
  private showWebcamClusterPopup(cam: any, clientX: number, clientY: number): void {
    const { tooltip } = this.makeWebcamTooltipShell();
    ...
    import('@/services/webcams').then(({ fetchWebcams }) => {
      fetchWebcams(10, {
        ...
        if (!tooltip.isConnected) return;
        const webcams = result.webcams.slice(0, 30);
